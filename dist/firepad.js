...
firepad.ParseHtml = (function () {
  ...
  function sanitizeFontSize(val) {
    ...
    if (firepad.utils.stringEndsWith(val, allowedValues)) {
      ...
      // no unit is converted to px with a 16x scale
      size = parseInt(val)*16+'px';
    }
    return size;
  }
      ...
      case Node.ELEMENT_NODE:
        ...
        switch (node.nodeName.toLowerCase()) {
          ...
          case 'font':
            ...
            if (size) { state = state.withTextFormatting(state.textFormatting.fontSize(sanitizeFontSize(size)));}
            parseChildren(node, state, output);
            break;
          case 'br':
